...
- **保存到此电脑**：主页底部大按钮（也可用顶栏「一键保存全部」/ 网格「保存所选」），按**游戏分文件夹**保存到 `桌面\SwitchAlbum\游戏名\`，主页显示当前路径、可随时更改
- **从电脑保存到手机**：主页底部大按钮进入本机相册页——浏览电脑上曾保存过的全部照片，勾选后传到安卓手机，手机里同样按游戏分文件夹（`DCIM\SwitchAlbum\游戏名`），图库立即可见；操作逻辑与从 Switch 保存一致（去重跳过、进度、已保存标记）
...
- **Switch 2 兼容**：文件夹名智能反查（自动剥离「Nintendo Switch 2 Edition」、DEMO、括号注释等后缀，支持日文/繁体/别名与包含匹配），封面命中率大幅提升
...
4. 点底部「保存到此电脑」或顶栏「一键保存全部」；之后点「从电脑保存到手机」，勾选要传的照片传到安卓手机
...
dotnet run --project SwitchAlbum.TestsHarness   # 纯函数与集成测试（39 项）
...
- 保存到手机仅支持安卓（MTP）。**iPhone 无法支持**：iOS 通过 USB 仅暴露只读导入模式（Windows 只能从 iPhone 读照片、无法写入相册），这是苹果的平台限制，任何第三方软件都无法绕过。替代思路：① iPhone 安装局域网传输类 App（照片需手动存入相册）；② 用 iCloud Drive 同步文件（文件 App 可见，但不进相册）
